...
const LoggedInMap = () => {
  ...
  const dummyActiveDrones = [
    ...
    { name: "Drone Bravo", position: [59.3375, 18.065], icon: icon },
  ];
  ...
  const devicesBottom =
    drawFlightBottom +
    (flightPathMenuOpen ? drawFlightPanelHeight : drawFlightButtonHeight) +
    10;
    ...
    <div style={{ position: "relative", height: "82vh", width: "100%" }}>
      ...
      >
        ...
        <GPSToggleControl
          ...
          toggleTracking={() => setTrackingEnabled((prev) => !prev)}
        />
        ...
        {showRestrictedZones &&
          dummyRestrictedZones.map((zone, idx) => (
            <Circle
              key={`restricted-${idx}`}
              center={zone.center}
              radius={zone.radius}
        ...
        {showActiveDrones &&
          dummyActiveDrones.map((drone, idx) => (
            <Marker
              key={`drone-${idx}`}
